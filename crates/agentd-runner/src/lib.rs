...
fn run_podman_command_with_input_until(
    args: Vec<String>,
    stdin_data: Option<&[u8]>,
    deadline: Instant,
) -> Result<Option<String>, RunnerError> {
...
fn terminate_child(child: &mut Child) -> Result<(), RunnerError> {
...
"#,
        ...
        assert!(
            ...
            "wait should re-check the attached child promptly after the timeout boundary, returned after {elapsed:?}"
        );
        assert_eq!(
            fixture.read_log("secret-commands.log"),
            "",
            "secret release should be skipped once the timeout boundary is reached"
        );
    }
...
"#,
        ...
        assert!(
            elapsed < Duration::from_millis(150),
            "wait should return promptly when inspect stalls past the deadline, returned after {elapsed:?}"
        );
        assert_eq!(
            fixture.read_log("secret-commands.log"),
            "",
            "secret release should be deferred to outer cleanup when inspect consumes the timeout window"
        );
